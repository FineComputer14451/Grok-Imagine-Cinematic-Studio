"""SFW generation mode decisions, cost estimates, and retry strategies."""
...
def estimate_shot_cost(
    ...
    *,
    ...
    if mode == "image_prompt":
        ...
        return {"mode": mode, "credits": usd_to_credits(usd), "usd": round(usd, 2)}
...
GENERATION_MODE_RULES: tuple[ModeRule, ...] = (
    ...
    (
        lambda s, _: _tier(s) in ("hero", "story_beat") and _has_ref(s),
        "image_to_video",
        "High-impact shot with reference — i2v for fidelity",
        0.88,
    ),
    (
        lambda s, _: _motion(s) == "low",
        "image_prompt",
        "Low motion — image is quota-efficient",
        0.85,
    ),
    (
