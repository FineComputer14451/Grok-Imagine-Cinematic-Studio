...
**Version:** 3.8.9  
**Status:** Canonical product manual  
...
14. [NSFW / R-Rated (Opt-in)](#14-nsfw--r-rated-opt-in)
15. [Repository Map](#15-repository-map)
16. [Examples & Templates](#16-examples--templates)
...
**Grok Imagine Cinematic Studio** is a complete, production-grade multi-agent framework that transforms creative ideas into locked, emotionally resonant cinematic sequences using:
...
- A **virtual film department**: Studio Director, specialists, readiness gates, and delivery polish
...
| **`cinematic-studio` CLI** | Bible, DNA, sequence, quota, handoff, plugins, doctor, validate |
| **Interactive TUI** | Live dashboard + safe launcher + production cockpit |
| **Streamlit Web UI** | Guided Bible, DNA bank, sequences, Imagine, cost estimation |
| **Plugin marketplace** | Full suite + 5 modular satellite packs |

Short product pitch: [OFFICIAL_OVERVIEW.md](OFFICIAL_OVERVIEW.md).  
...
| **Grok 4.5 access** | Cinematic default; optional Grok 4.3 for 1M |
...
Surfaces that **do not** host the Grok Build binary: grok.com web chat and the Grok mobile app. Use chat Activate / `MASTER_PROMPT.md` there; use shell for full CLI.
...
8. Sequence Director + specialists (Stunts, VFX, Sound, Performance, etc.).
...
├── .grok/skills/              # Skill runtime (SKILL.md per skill)
...
TUI + Streamlit **view density modes** (compact / ops / full) on top of the v3.8.8 operator control plane.
