...
ACTIONS: dict[str, ActionSpec] = {
    ...
    "imagine_poll": ActionSpec(
        id="imagine_poll",
        label="Poll video request",
        description="GET /v1/videos/{request_id} (one shot, no --wait)",
        base_argv=("imagine", "poll"),
        surfaces=frozenset({"launcher", "cockpit"}),
        needs_confirm=False,
        group="files",
        fields=(
            _f("request_id", "Video request_id", required=True),
            _f("job_id", "Studio job id to update (optional)", flag="--job-id"),
        ),
    ),
    ...
    "files_share": ActionSpec(
        id="files_share",
        label="Files share (public URL)",
        description="POST /v1/files/{id}/public-url — CDN link (idempotent)",
        base_argv=("files", "share"),
        surfaces=frozenset({"cockpit"}),
        needs_confirm=False,
        group="files",
        fields=(
            _f("file_id", "Files API id", required=True),
            _f(
                "expires_after",
                "URL TTL seconds (3600–2592000)",
                flag="--expires-after",
                coerce="int",
            ),
        ),
    ),
    "files_unshare": ActionSpec(
        id="files_unshare",
        label="Files unshare",
        description="POST /v1/files/{id}/public-url/revoke — file stays",
        base_argv=("files", "unshare"),
        surfaces=frozenset({"cockpit"}),
        needs_confirm=True,
        group="files",
        fields=(_f("file_id", "Files API id", required=True),),
    ),
