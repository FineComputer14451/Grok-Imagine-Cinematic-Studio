...
def _mode_radio() -> str:
    """TUI-parity density: compact / ops / full (session-persisted)."""
    labels = {
        "compact": "1 · Compact",
        "ops": "2 · Ops",
        "full": "3 · Full",
    }
    current = dui.normalize_dashboard_mode(st.session_state.get("dashboard_view_mode", "ops"))
    choice = st.radio(
        "Dashboard view (TUI Home parity)",
        options=list(labels.keys()),
        format_func=lambda k: labels[k],
        horizontal=True,
        index=["compact", "ops", "full"].index(current),
        key="dashboard_view_mode_radio",
        help="Compact = orient · Ops = gates + quota · Full = all tables (matches TUI 1/2/3)",
    )
    st.session_state["dashboard_view_mode"] = choice
    return dui.normalize_dashboard_mode(choice)


def _show(mode: str, section: str) -> bool:
    return dui.section_visible(mode, section)
...
def render() -> None:
    ...
    top_l, top_mid, top_r = st.columns([2, 2, 1])
    with top_l:
        mode = _mode_radio()
    with top_mid:
        st.caption(
            f"View **{mode}** · TUI twin: `cinematic-studio ui` keys **1**/**2**/**3**"
        )
    with top_r:
    ...
    # KPI metrics (always — orient at a glance)
    title = project["title"]
    ...
    if _show(mode, "health_actions"):
        ...
        if health:
            htitle = health.get("title", "command")
            code = int(health.get("code") or 1)
            out = str(health.get("out") or "(no output)")
            if code == 0:
                st.success(f"`{htitle}` · exit {code}")
            else:
                st.warning(f"`{htitle}` · exit {code}")
            with st.expander(f"Output: {htitle}", expanded=True):
                st.code(out, language="text")

    # Readiness
    if _show(mode, "readiness"):
        ...
        if readiness:
            ...
            if next_actions:
                ...
                st.caption(
                    "Gate clear for careful video spend (still prefer strict flags on CLI)."
                )
        else:
            st.caption("Readiness rollup unavailable — refresh snapshot.")

    # Convergence
    if _show(mode, "convergence"):
    ...
    # Parallel briefs (full, or ops if logs exist)
    pb = snap.get("parallel_briefs") or {}
    logs = pb.get("logs") or []
    if _show(mode, "briefs") or (mode == "ops" and logs):
                ...
                [
                    {
                        ...
                        "Blocker": (
                            r.get("polish_blockers") or r.get("deliver_blockers") or ["—"]
                        )[0],
                    }
                    for r in drows
                ],
    ...
    if _show(mode, "studio_quota"):
        ...
        with col_l:
            ...
            st.markdown(
                ...
                f"- **SFW / NSFW batches:** {prod.get('sfw_batches', 0)} / "
                f"{prod.get('nsfw_batches', 0)}\n"
    ...
    if _show(mode, "chain_qa"):
        ...
        if qa_rows:
            ...
            if any(int(r.get("No-Go") or 0) > 0 for r in qa_rows):
                ...
                st.caption(
                    "All listed sequences free of no-go · validate handoff packets before stitch."
                )
        else:
            st.caption(
                "No sequence QA data yet · generate clips then run chain QA before extend."
            )

    if _show(mode, "characters"):
    ...
    if _show(mode, "spend") and quota.get("recent_history"):
    ...
    st.info(
        ...
        f"View **{mode}** · Terminal twin: `cinematic-studio ui` (**1**/**2**/**3**)"
    )
